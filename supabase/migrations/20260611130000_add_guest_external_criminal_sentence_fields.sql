alter table public.case_alloggio_submissions
  add column if not exists in_esecuzione_penale_esterna text null,
  add column if not exists esecuzione_penale_esterna_data_inizio text null,
  add column if not exists esecuzione_penale_esterna_data_fine text null;
...
begin
  ...
  v_current_status := public.normalize_case_alloggio_current_status(
    v_guest.current_status,
    v_guest.data_uscita,
    v_guest.data_decesso,
    v_guest.tipo_aggiornamento
  );

  if p_patch ? 'data_decesso' and v_current_status <> 'DECEDUTO' then
    raise exception 'La data decesso e modificabile solo per ospiti deceduti' using errcode = '42501';
  end if;

  select coalesce(jsonb_object_agg(key, value), '{}'::jsonb)
  into v_old
  from jsonb_each(to_jsonb(v_guest))
  ...
  foreach v_key in array v_allowed loop
    if p_patch ? v_key then
      v_new := jsonb_set(
        v_new,
        array[v_key],
        coalesce(to_jsonb(nullif(btrim(coalesce(p_patch->>v_key, '')), '')), 'null'::jsonb),
        true
      );
    end if;
  end loop;
  ...
  from unnest(v_allowed) as key;

  execute format(
    'update public.case_alloggio_submissions set %s where id = $1',
    v_update_assignments
  )
  using p_guest_id, v_new;
