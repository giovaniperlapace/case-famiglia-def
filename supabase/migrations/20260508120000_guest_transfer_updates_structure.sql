...
declare
  ...
  v_struttura_trasferimento text := nullif(btrim(coalesce(v_payload->>'struttura_trasferimento', '')), '');
begin
  select * into v_guest
  ...
  if v_struttura_trasferimento is not null then
    if v_from_status <> 'IN_ACCOGLIENZA' or p_to_status <> 'IN_ACCOGLIENZA' then
      raise exception 'Transfers are allowed only for guests in accoglienza';
    end if;

    if lower(v_struttura_trasferimento) = lower(coalesce(v_guest.struttura, '')) then
      raise exception 'Transfer target must differ from current struttura';
    end if;

    if v_struttura_trasferimento <> all(array[
      'Buon Pastore',
      'San Calisto',
      'Villetta',
      'Palazzo Migliori',
      ...
      raise exception 'Invalid transfer target struttura';
    end if;
  end if;
  ...
  if p_to_status is not null and (p_to_status <> v_from_status or v_struttura_trasferimento is not null) then
    update public.case_alloggio_submissions
    set
      current_status = p_to_status,
      current_status_at = v_effective,
      struttura = case
        when v_struttura_trasferimento is not null then v_struttura_trasferimento
